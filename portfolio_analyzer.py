...
class PortfolioAnalyzer:
    ...
    def oil_exposure(self, oil_proxy: str = "USO") -> dict:
        """
        Estimate portfolio oil exposure from historical return covariance.

        Returns covariance statistics and the portfolio beta to the chosen
        oil proxy ETF. Beta is defined as Cov(portfolio, oil) / Var(oil).
        """
        oil = self.benchmark_returns(oil_proxy)
        data = pd.concat([self.portfolio_returns, oil], axis=1).dropna()
        if data.empty:
            return {
                "oil_proxy": oil_proxy,
                "covariance": 0.0,
                "annualized_covariance": 0.0,
                "beta": 0.0,
                "correlation": 0.0,
                "n_obs": 0,
            }
        ...
        port = data.iloc[:, 0]
        oil_ret = data.iloc[:, 1]
        cov = float(port.cov(oil_ret))
        oil_var = float(oil_ret.var())
        corr = float(port.corr(oil_ret)) if len(data) > 1 else 0.0
        ppy = self._periods_per_year()

        return {
            "oil_proxy": oil_proxy,
            "covariance": cov,
            "annualized_covariance": cov * ppy,
            "beta": float(cov / oil_var) if oil_var > 1e-12 else 0.0,
            "correlation": corr if not np.isnan(corr) else 0.0,
            "n_obs": int(len(data)),
        }

    def rates_exposure(self, rates_proxy: str = "^TNX") -> dict:
        """
        Estimate portfolio interest-rate exposure from historical covariance.

        Uses the chosen rates proxy's historical return series. With the default
        ^TNX proxy, a positive beta means the portfolio has historically moved
        with rising 10Y Treasury yields.
        """
        rates = self.benchmark_returns(rates_proxy)
        data = pd.concat([self.portfolio_returns, rates], axis=1).dropna()
        if data.empty:
            return {
        ...
        port = data.iloc[:, 0]
        rates_ret = data.iloc[:, 1]
        cov = float(port.cov(rates_ret))
        rates_var = float(rates_ret.var())
        corr = float(port.corr(rates_ret)) if len(data) > 1 else 0.0
        ppy = self._periods_per_year()
        ...
        return {
            ...
            "covariance": cov,
            "annualized_covariance": cov * ppy,
            "beta": float(cov / rates_var) if rates_var > 1e-12 else 0.0,
            "correlation": corr if not np.isnan(corr) else 0.0,
            "n_obs": int(len(data)),
        }
